...
app.forms = {
	...
	init(form) {
		...
		form.find('.'+_this.sel.field+_this.sel.fieldReq).each(function () {
			$(this).append(`<div class="${_this.sel.errors}"><p class="${_this.sel.errors+_this.sel.errorsItem} ${_this.sel.errors+_this.sel.errorsItem+_this.sel.errorsReq}">Обязательное поле</p></div>`);
		});
		...
		form.on('submit', function (e) {
			...
			if (valid) {
				...
				if (!app.options.formTitle || app.options.formTitle == '') {
					app.options.formTitle = 'Заявка';
				}
				formData.append('formTitle', app.options.formTitle);
